import { Table, TableInfo } from './table';
...
export class Query {
  ...
  constructor(tables?: Table[] | Table | TableInfo[] | TableInfo) {
    if (tables) {
      const tableArray = Array.isArray(tables) ? tables : [tables];
      for (const table of tableArray) {
        const info = table instanceof Table ? table.build() : table;
        this.tables.set(info.name, info);
      }
    }
  }
  ...
  or(column: string): this {
    if (this.whereConditions.length > 0) {
      this.whereConditions[this.whereConditions.length - 1].logical = 'OR';
    }
    this.currentColumn = column;
    return this;
  }
