...
Obtain and copy into `apps\web\public` a 512x512 `logo.svg` and `favicon.ico`

## Title

Update title in `apps\web\package.json`
    ...
    <v-menu v-if="!mobile">
      ...
      <v-list>
        <v-list-item v-for="(item, i) in app.settings.fontSizes" :key="i" :value="i">
          <v-list-item-title @click="app.settings.setFontSize(item)">{{ item }}%</v-list-item-title>
        </v-list-item>
      </v-list>
    </v-menu>
    <v-menu v-if="!mobile">
