...
export const registerDbScaffoldCommand = (program: Command) => {
  program
    ...
    .action(
      async (
        ...
        options: { output?: string; idempotent?: boolean; run?: boolean },
      ) => {
        try {
        ...
        async function scanAndExportSchemas(
          idempotent: boolean,
          promptRun: boolean,
        ): Promise<void> {
  ...
  console.log(JSON.stringify({ success: false, error: err.message }));
              ...
              try {
                ...
                logger.success(
                  `Exported ${output.schemaName} → ${path.relative(rootDir, outputPath)}`,
                )
                ...
                if (output.sqlStatements && output.sqlStatements.length > 0) {
                    ...
                    if (type === 'comment') {
                      ...
                      for (const stmt of statements) {
                        ...
                        if (stmt.name.endsWith('_table')) {
                          ...
                          const tableCommentName = statements.find((s) =>
                            s.name.endsWith('_table'),
                          )?.name
                          if (tableCommentName) {
                            ...
                            tableName =
                              lastUnderscore > 0 ? stmt.name.slice(0, lastUnderscore) : stmt.name
                          }
                        }
                  ...
                  logger.success(
                    `Generated ${sqlFileCount} SQL file(s) → ${path.relative(rootDir, releasesDir)}/`,
                  )
                  logger.success(
                    `Created master script → ${path.relative(rootDir, masterScriptPath)}`,
                  )
                  if (idempotent) {
              ...
              logger.error(
                `Failed to process ${path.relative(rootDir, apiFile)}: ${error instanceof Error ? error.message : String(error)}`,
              )
            }
          }
