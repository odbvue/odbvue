...
import { logger, rootDir, path, writeFileSync, unlinkSync, execSync, platform } from '../utils.js'
  ...
  program
    ...
    .action(async (options: { connection?: string }) => {
      try {
        ...
        if (!connection) {
          logger.error(
            'Database connection not provided and ODBVUE_DB_CONN environment variable not set.',
          )
