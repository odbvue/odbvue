  ...
  if (diff.modifiedTables.length > 0) {
    ...
    for (const mod of diff.modifiedTables) {
      ...
      for (const col of mod.newColumns) {
        ...
        lines.push(`BEGIN`);
        lines.push(`  SELECT COUNT(*) INTO v_exists FROM all_tab_columns WHERE owner = '${schemaName}' AND table_name = '${mod.name}' AND column_name = '${col.name}';`);
      ...
      for (const idx of mod.newIndexes) {
        ...
        lines.push(`  v_exists NUMBER;`);
        lines.push(`BEGIN`);
        lines.push(`  SELECT COUNT(*) INTO v_exists FROM all_indexes WHERE owner = '${schemaName}' AND index_name = '${idxName.toUpperCase()}';`);
        lines.push(`  IF v_exists = 0 THEN`);
        lines.push(`    EXECUTE IMMEDIATE 'CREATE INDEX ${schemaName}.${idxName} ON ${fullTableName} (${idx.join(', ')})';`);
...
function generateIdempotentCreateTable(schemaName: string, table: TableInfo): string {
  ...
  if (table.unique) {
    for (const cols of table.unique) {
      ...
      lines.push(`  v_exists NUMBER;`);
      lines.push(`BEGIN`);
      lines.push(`  SELECT COUNT(*) INTO v_exists FROM all_constraints WHERE owner = '${schemaName}' AND constraint_name = '${ukName.toUpperCase()}';`);
      lines.push(`  IF v_exists = 0 THEN`);
      lines.push(`    EXECUTE IMMEDIATE 'ALTER TABLE ${fullTableName} ADD CONSTRAINT ${ukName} UNIQUE (${cols.join(', ')})';`);
  ...
  if (table.checks) {
    for (const chk of table.checks) {
      ...
      lines.push(`  v_exists NUMBER;`);
      lines.push(`BEGIN`);
      lines.push(`  SELECT COUNT(*) INTO v_exists FROM all_constraints WHERE owner = '${schemaName}' AND constraint_name = '${ckName.toUpperCase()}';`);
      lines.push(`  IF v_exists = 0 THEN`);
      lines.push(`    EXECUTE IMMEDIATE 'ALTER TABLE ${fullTableName} ADD CONSTRAINT ${ckName} CHECK (${chk.column} IN (${values}))';`);
  ...
  if (table.indexes) {
    for (const cols of table.indexes) {
      ...
      lines.push(`  SELECT COUNT(*) INTO v_exists FROM all_indexes WHERE owner = '${schemaName}' AND index_name = '${idxName.toUpperCase()}';`);
      lines.push(`  IF v_exists = 0 THEN`);
      lines.push(`    EXECUTE IMMEDIATE 'CREATE INDEX ${schemaName}.${idxName} ON ${fullTableName} (${cols.join(', ')})';`);
      lines.push(`    DBMS_OUTPUT.PUT_LINE('Created index ${idxName}');`);
      lines.push(`  END IF;`);
      lines.push(`END;`);
      lines.push(`/`);
  ...
  if (offlineMode) {
    ...
    if (!user || !password) {
      ...
      try {
        ...
        logger.warn(`Could not connect to database: ${error instanceof Error ? error.message : String(error)}`);
