...
export const registerDbInstallLocalCommand = (program: Command) => {
  program
    ...
    .alias('dil')
    .description(
      'Install/upgrade schema + objects into the local DB using db/dist and db/.config.json',
    )
    .option(
      '-c, --connection <connection>',
      'Database connection (uses ODBVUE_DB_CONN if not provided)',
    )
    .option(
      '-t, --tns-admin <path>',
      'TNS_ADMIN directory path (uses ODBVUE_TNS_ADMIN if not provided)',
    )
    ...
    .action((options: { connection?: string; tnsAdmin?: string; version?: string }) => {
      ...
      if (!connection) {
        logger.error(
          'Database connection not provided and ODBVUE_DB_CONN not set (try: ov local-setup).',
        )
        process.exit(1)
      }
      ...
      if (!existsSync(dbConfigPath)) {
        logger.error(
          'Missing db/.config.json. Create it from db/.config.json.example (or run ov local-setup).',
        )
        process.exit(1)
      }
      ...
      logger.info(
        `Installing to schema '${schemaName}', edition '${edition}', version '${versionTag}'...`,
      )
