...
import VOvMedia from '../../src/capabilities/ui/components/VOvMedia.vue'
...
beforeAll(() => {
  ...
  HTMLMediaElement.prototype.play = vi.fn<() => Promise<void>>()
  HTMLMediaElement.prototype.pause = vi.fn<() => void>()

  // Mock canvas methods
  HTMLCanvasElement.prototype.getContext = vi
    .fn<() => CanvasRenderingContext2D | null>()
    .mockImplementation(
      () => new MockCanvasContext() as unknown as CanvasRenderingContext2D,
    ) as never
  HTMLCanvasElement.prototype.toDataURL = vi.fn<() => string>(() => 'data:image/png;base64,test')
  HTMLCanvasElement.prototype.toBlob = vi.fn<(callback: BlobCallback) => void>((callback) => {
