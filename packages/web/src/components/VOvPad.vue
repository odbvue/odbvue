...
const handleResize = async () => {
  ...
  const targetCanvas = document.querySelector(`#${props.canvasId}`) as HTMLCanvasElement
  if (targetCanvas) {
...
const getCoordinates = (event: MouseEvent | TouchEvent) => {
  let x = 0,
    y = 0
  const canvasElement = document.querySelector(`#${props.canvasId}`) as HTMLCanvasElement
...
const drawShape = (
  ...
  canvasContext.strokeStyle = strokes.color
  canvasContext.fillStyle = strokes.color
  canvasContext.lineWidth = strokes.width
  canvasContext.lineJoin = strokes.lineJoin || props.lineJoin
  canvasContext.lineCap = strokes.lineCap || props.lineCap
  ...
  if (
    ...
    canvasContext.moveTo(strokes.from.x, strokes.from.y)
