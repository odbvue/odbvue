...
const LOB_PKS = `CREATE OR REPLACE PACKAGE pck_api_lob AS -- Package for LOB processing. Credit: https://github.com/paulzip-dev/Base64
...
const LOB_PKB = `CREATE OR REPLACE PACKAGE BODY pck_api_lob AS
...
export const odbLob = {
  ...
  toSQLUp(options: { schema?: string } = {}): string {
    if (!options.schema) return `${LOB_PKS}\n${LOB_PKB}`
    ...
    return [
      LOB_PKS.replace(/^CREATE OR REPLACE PACKAGE pck_api_lob AS/, specHeader),
      LOB_PKB.replace(/^CREATE OR REPLACE PACKAGE BODY pck_api_lob AS/, bodyHeader),
    ].join('\n')
  },
