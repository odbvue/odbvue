...
export const odbRateLimitApi = odbPackage('odb_rate_limit', (pkg) => {
  ...
  const enforce = pkg.defineProcedure('enforce', {
    in: { scope: odbType.string(), subject: odbType.string() },
  })
  enforce.body((body) => {
    const { scope, subject } = enforce.parameters
  ...
  const failure = pkg.defineProcedure('failure', {
    in: { scope: odbType.string(), subject: odbType.string() },
  })
  failure.autonomous().body((body) => {
    const { scope, subject } = failure.parameters
  ...
  const success = pkg.defineProcedure('success', {
    in: { scope: odbType.string(), subject: odbType.string() },
  })
  success.autonomous().body((body) => {
    const { scope, subject } = success.parameters
  ...
  return {
    hashSubject,
    enforce: enforce.procedure,
    failure: failure.procedure,
    success: success.procedure,
  }
})
