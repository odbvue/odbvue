...
-- changeset ODBVUE:1762284800747 stripComments:false  logicalFilePath:featdb\odbvue\package_bodies\pck_api_http.sql
-- sqlcl_snapshot db/src/database/odbvue/package_bodies/pck_api_http.sql:null:c0ed4c0bf0ff96aca1fbf570b9d20a3b5bf18db9:create
...
CREATE OR REPLACE PACKAGE BODY odbvue.pck_api_http AS
    ...
    BEGIN
        IF c_debug = 1 THEN
            dbms_output.put_line('Authorization: Basic '
                                 || p_username
                                 || ':' || regexp_replace(p_password, '-\w', '*'));
        END IF;
    ...
    BEGIN
        IF c_debug = 1 THEN
            dbms_output.put_line('Authorization: Bearer ' || regexp_replace(p_token, '\w', '*'));
        END IF;
    ...
    BEGIN
        IF c_debug = 1 THEN
            dbms_output.put_line('Authorization: Wallet file:'
                                 || p_wallet_path
                                 || ' : ' || regexp_replace(p_wallet_password, '\w', '*'));
        END IF;
    ...
    BEGIN
        IF p_content_type IS NOT NULL THEN
            IF c_debug = 1 THEN
                dbms_output.put_line('Header content-type: ' || p_content_type);
            END IF;
    ...
    BEGIN
        IF p_body_charset IS NOT NULL THEN
            IF c_debug = 1 THEN
                dbms_output.put_line('Charset: ' || p_body_charset);
            END IF;
    ...
    BEGIN
        IF c_debug = 1 THEN
            dbms_output.put_line('Content-Type: multipart/form-data; boundary="'
                                 || c_boundary
                                 || '"' ||
                CASE
                    WHEN p_charset IS NOT NULL THEN
                        '; charset=UTF-8'
                    ELSE
                        ''
                END
        ...
        utl_http.set_header(p_req, 'Content-Type', 'multipart/form-data; boundary="'
                                                   || c_boundary
                                                   || '"'
                                                   ||
            CASE
    ...
    PROCEDURE request_multipart_blob (
        ...
        v_chunk  CLOB := ''
                        ...
                        || utl_tcp.crlf
                        || 'Content-Type: '
                        || mime_type(substr(p_filename,
                                            nullif(instr(p_filename, '.', -1) + 1,
                                                   1)))
                        || utl_tcp.crlf
                        || utl_tcp.crlf;
        v_offset NUMBER := 1;
    ...
    BEGIN
        ...
        utl_http.write_text(p_req, v_chunk);
        IF c_debug = 1 THEN
            dbms_output.put_line('Writing out '
    ...
    BEGIN
        ...
        utl_http.write_text(p_req, utl_tcp.crlf
                                   || '--'
                                   || c_boundary
                                   || '--');
    ...
    BEGIN
        ...
        BEGIN
            LOOP
                ...
                IF c_debug = 1 THEN
                    dbms_output.put_line(length(v_chunk) || ' bytes..');
                END IF;
    ...
    BEGIN
        ...
        BEGIN
            LOOP
                ...
                IF c_debug = 1 THEN
                    dbms_output.put_line(utl_raw.length(v_chunk) || ' bytes..');
                END IF;

                dbms_lob.writeappend(v_result,
                                     utl_raw.length(v_chunk),
                                     v_chunk);
            END LOOP;
