...
-- changeset ODBVUE:hotfix_pck_api_openai stripComments:false runOnChange:true logicalFilePath:hotfix\odbvue\package_specs\pck_api_openai.sql
...
CREATE OR REPLACE PACKAGE odbvue.pck_api_openai AS -- Package provides implementation of Open AI API
    ...
    PROCEDURE speech ( -- Procedure generates speech from text
        p_api_key         VARCHAR2, -- OpenAI API Key
        p_model           VARCHAR2, -- Model (tts-1, tts-1-hd)
    ...
    PROCEDURE speech ( -- Procedure generates speech from text (simplified)
        p_api_key VARCHAR2, -- OpenAI API Key
        p_input   VARCHAR2, -- Text
        r_speech  OUT BLOB, -- Audio file
        r_error   OUT VARCHAR2 -- Error message
    ...
    PROCEDURE transcript ( -- Procedure transcripts audio file
        p_api_key         VARCHAR2, -- OpenAI API Key
        p_file            BLOB, -- File
    ...
    PROCEDURE translations ( -- Procedure translates audio file
        p_api_key         VARCHAR2, -- OpenAI API Key
        p_file            BLOB, -- File
        p_filename        VARCHAR2, --  Filename
        p_model           VARCHAR2, -- Model (whisper-1)
    ...
    PROCEDURE completion ( -- Procedure serves dialog with Open AI
        p_api_key VARCHAR2, -- OpenAI API Key
        p_model   VARCHAR2, -- Model (gpt-4, gpt-4-turbo, gpt-3.5-turbo, etc.)
        p_prompt  VARCHAR2, -- Prompt
    ...
    PROCEDURE moderations ( -- Procedure provides moderation
        p_api_key     VARCHAR2, -- OpenAI API Key
    ...
    PROCEDURE vision ( -- Procedure provides image analysis capabilities
        p_api_key VARCHAR2, -- OpenAI API Key
    ...
    PROCEDURE image ( -- Procedure generates images
        p_api_key VARCHAR2, -- OpenAI API Key
