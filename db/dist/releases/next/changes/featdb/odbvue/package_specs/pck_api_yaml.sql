-- liquibase formatted sql
-- changeset ODBVUE:1762637561577 stripComments:false  logicalFilePath:featdb\odbvue\package_specs\pck_api_yaml.sql
-- sqlcl_snapshot db/src/database/odbvue/package_specs/pck_api_yaml.sql:null:981a0251d087b9a1374cd606074ac6e5df561eac:create

CREATE OR REPLACE PACKAGE odbvue.pck_api_yaml IS -- Package for handling YAML

    FUNCTION init -- Function to initialize empty yaml object 

     RETURN CLOB; -- Returns a clob representing an empty yaml object

    FUNCTION EXISTS ( -- Function to check if a path exists in a yaml object
        c IN CLOB, -- The yaml object as clob
        p IN VARCHAR2 -- The path to check for existence
    ) RETURN BOOLEAN; -- Returns true if the key exists, false otherwise

    FUNCTION read ( -- Function reads by path form a yaml object
        c IN CLOB, -- The yaml object as clob
        p IN VARCHAR2 -- The path to read
    ) RETURN CLOB; -- Returns the value at the specified path as clob

    FUNCTION typeof ( -- Function to get the type of the value at a specified path
        c IN CLOB, -- The yaml object as clob
        p IN VARCHAR2 -- The path to check the type of
    ) RETURN VARCHAR2; -- Returns the type as a string

    FUNCTION elcount ( -- Function to count elements in an array at a specified path
        c IN CLOB, -- The yaml object as clob
        p IN VARCHAR2 -- The path to the array
    ) RETURN PLS_INTEGER; -- Returns the count of elements

    FUNCTION keys ( -- Function to get the keys of an object at a specified path
        c IN CLOB, -- The yaml object as clob
        p IN VARCHAR2 -- The path to the object
    ) RETURN VARCHAR2; -- Returns a list of keys

    PROCEDURE write ( -- Procedure to replace a value at a specified path 
        c IN OUT NOCOPY CLOB, -- The yaml object as clob 
        p IN VARCHAR2, -- The path to insert the value at (object path only, arrays not supported yet)
        v IN CLOB -- The value to insert
    );

    PROCEDURE print ( -- Procedure to pretty-print a yaml object
        c IN OUT NOCOPY CLOB -- The yaml object as clob
    );

    FUNCTION to_xml ( -- Function to convert yaml object to xml representation
        c CLOB -- The yaml object as clob
    ) RETURN CLOB; -- Returns the xml representation as clob

    FUNCTION to_json ( -- Function to convert yaml object to JSON representation
        c IN CLOB --- The yaml object as clob
    ) RETURN CLOB; -- Returns the JSON representation as clob
END pck_api_yaml;
/

