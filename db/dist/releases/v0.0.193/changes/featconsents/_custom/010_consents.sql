...
-- changeset  SqlCl:1763710578602 stripComments:false logicalFilePath:featconsents\_custom\010_consents.sql
-- sqlcl_snapshot dist\releases\next\changes\featconsents\_custom\010_consents.sql:null:null:custom

SET DEFINE OFF;

MERGE INTO app_consents ac
USING (
    SELECT 'en' AS language_id, 'OdbVue Terms & Conditions' AS name, 1 AS seq FROM dual
    UNION ALL
    SELECT 'de' AS language_id, 'OdbVue Allgemeine Geschäftsbedingungen' AS name, 2 AS seq FROM dual
    UNION ALL
    SELECT 'fr' AS language_id, 'OdbVue Conditions Générales' AS name, 3 AS seq FROM dual
...
WHEN NOT MATCHED THEN
    INSERT (
        ...
        source.language_id,
        source.name,
        CASE source.language_id
            WHEN 'en' THEN q'[
# OdbVue Demo Application --- Terms & Conditions
...
terms, you must stop using the Service.
            ...
            WHEN 'de' THEN q'[
# OdbVue Demoanwendung --- Allgemeine Geschäftsbedingungen
...
Dienstes einstellen.
            ...
            WHEN 'fr' THEN q'[
# OdbVue Application Démonstration --- Conditions Générales
